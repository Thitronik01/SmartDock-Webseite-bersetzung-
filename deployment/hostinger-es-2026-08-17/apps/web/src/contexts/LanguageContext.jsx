...
import { sourceTranslations, translations as staticTranslations } from '@/utils/translations/index.js';
import { COUNTRIES } from '@/translations.js';
import { getLangFromPath, getEquivalentPath } from '@/config/routes.js';
import { allowsGermanFallback, getContentLocaleCode, isSelectableLocale } from '@/config/locales.js';
...
export const LanguageProvider = ({ children }) => {
  ...
  // The URL is the single source of truth for every live locale.
  const currentLanguage = getLangFromPath(location.pathname);
  ...
  const setCurrentLanguage = (lang) => {
    ...
    if (!isSelectableLocale(upperLang)) return;
  ...
  const t = useCallback((key, options = {}) => {
    ...
    const langKey = getContentLocaleCode(currentLanguage);
    ...
    // Live locales retain the existing German safety net. Preview locales
    // deliberately expose missing keys instead of leaking German copy.
    if (allowsGermanFallback(currentLanguage)) {
      for (const lookupKey of lookupKeys) {
  ...
  const translateText = useCallback(async (text, requestedLanguage = currentLanguage) => {
    ...
    const langKey = getContentLocaleCode(requestedLanguage);
    ...
    const sourceValue = sourceTranslations[langKey]?.[normalized];
    if (typeof sourceValue === 'string' && sourceValue.trim()) return sourceValue;

    return allowsGermanFallback(requestedLanguage) ? text : `[${langKey} missing]`;
