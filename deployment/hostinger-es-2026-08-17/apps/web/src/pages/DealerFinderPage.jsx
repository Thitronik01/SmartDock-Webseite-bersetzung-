  ...
  useEffect(() => {
    const fetchDealers = async () => {
      ...
      try {
        ...
        setError(currentLanguage === 'ES'
          ? 'No se han podido cargar los distribuidores. Inténtalo de nuevo más tarde.'
          : 'Händler konnten nicht geladen werden. Bitte versuchen Sie es später erneut.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background pt-28 pb-20 flex flex-col">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex-grow flex flex-col">
        ...
        <Breadcrumb 
          items={[
            { label: currentLanguage === 'ES' ? 'Inicio' : isEn ? 'Home' : 'Startseite', path: pathFor('home', currentLanguage) },
          ...
          <div className={`w-[800px] h-[800px] shrink-0 rounded-2xl overflow-hidden border border-border relative z-0 ${activeTab === 'list' ? 'hidden md:block' : 'block'}`}>
            {loading ? (
              ...
              <MapContainer center={[51.1657, 10.4515]} zoom={5} style={{ height: "100%", width: "100%" }} className="z-0 bg-muted">
                ...
                {userLocation && <Marker position={[userLocation.lat, userLocation.lng]} icon={createCustomIcon(true)}><Popup>{currentLanguage === 'ES' ? t('df_user_location') : 'Ihr Standort'}</Popup></Marker>}
                {dealersWithCoords.map((dealer) => (
                  <Marker 
