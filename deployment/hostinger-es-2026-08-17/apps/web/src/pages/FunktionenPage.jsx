...
const FunktionenPage = () => {
  ...
  const htmlLang = isEs ? 'es-ES' : isEn ? 'en-US' : isFr ? 'fr-FR' : 'de-DE';
  ...
  return (
    <div className="min-h-screen bg-background pt-28 pb-20">
      <Helmet>
        <html lang={htmlLang} />
        <title>{isEs ? 'Funciones - SMARTDOCK' : isEn ? `Features - SMARTDOCK` : isFr ? `Fonctions - SMARTDOCK` : `Funktionen - SMARTDOCK`}</title>
        <meta 
          name="description" 
          content={isEs
            ? 'Descubre todas las funciones de la app SMARTDOCK, desde One Hand Mode hasta el control del ancla.'
            : isEn
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Breadcrumb 
          items={[
            { label: isEs ? 'Inicio' : isEn ? 'Home' : isFr ? 'Accueil' : 'Startseite', path: pathFor('home', currentLanguage) },
            { label: isEs ? 'Funciones' : isEn ? 'Features' : isFr ? 'Fonctions' : 'Funktionen', path: pathFor('functions', currentLanguage) }
          ]} 
        />
