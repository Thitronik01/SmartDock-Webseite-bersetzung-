...
const GuideItem = ({ guide, lang }) => {
  ...
  const title = isEs
    ? translateSourceText(guide.titel, 'ES')
    : isFr
  ...
  let description;
  if (isEs) {
    description = guide.beschreibung ? translateSourceText(guide.beschreibung, 'ES') : '';
  } else if (isFr && guide.beschreibung_fr && guide.beschreibung_fr.trim() !== '') {
  ...
  const downloadLabel = isEs ? 'Descargar PDF' : (isFr ? 'Télécharger le PDF' : (isEn ? 'Download PDF' : 'PDF Herunterladen'));
  const downloadAriaLabel = isEs
...
const InstallationPage = () => {
  ...
  const tt = (de, en, fr) => (isEs ? translateSourceText(de, 'ES') : isFr ? (fr ?? en ?? de) : isEn ? (en ?? de) : de);
  ...
  return (
    <div className="min-h-screen bg-background pt-28 pb-20">
      <Helmet>
        <html lang={isEs ? 'es-ES' : isFr ? "fr-FR" : isEn ? "en-US" : "de-DE"} />
        <title>{isEs ? 'Instalación - SMARTDOCK' : 'Installation - SMARTDOCK'}</title>
        <meta
          name="description"
          content={tt(
