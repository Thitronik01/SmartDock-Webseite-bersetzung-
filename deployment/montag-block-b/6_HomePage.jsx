...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    // the visible URL.
    const isContactRoute = location.pathname === '/en/contact' || location.pathname === '/fr/contact' || location.pathname === '/ch/contact' || location.pathname === '/es/contacto';
  ...
  return (
    <div className="bg-background w-full flex-grow flex flex-col">
      <Helmet>
        <html lang={currentLanguage === 'ES' ? 'es-ES' : 'de-DE'} />
        <title>{currentLanguage === 'ES' ? 'SMARTDOCK - Solución inteligente para maniobras en puerto' : 'SMARTDOCK - Intelligente Hafenmanöver-Lösung'}</title>
        <meta name="description" content={currentLanguage === 'ES' ? t('sd_hero_subtitle') : 'Vollständige Kontrolle und Sicherheit bei jedem Manöver. Entwickelt für anspruchsvolle Kapitäne, die auf Präzision und Innovation setzen.'} />
      </Helmet>
