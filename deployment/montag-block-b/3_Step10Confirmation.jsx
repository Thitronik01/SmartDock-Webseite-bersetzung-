...
const Step10Confirmation = () => {
  ...
  // Spanisch liest aus dem ES-Wörterbuch. DE, EN und FR behalten unverändert
  // den bisherigen Text, damit die Live-Sprachen keine neuen Schlüssel brauchen.
  const esText = (key, fallback) => (currentLanguage === 'ES' ? t(key) : fallback);
  ...
  if (!configurationRecord) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center space-y-6">
        <p className="text-xl text-muted-foreground">{esText('conf_succ_data_error', 'Daten konnten nicht geladen werden.')}</p>
    ...
    <div className="w-full max-w-4xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500 pb-16">
      <div className="text-center space-y-4 pt-8">
        ...
        <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
          {selectedDealer ? t('conf_succ_desc') : esText('conf_succ_desc_no_dealer', 'Der ausgewählte Händler wird sich in Kürze bei Ihnen melden.')}
        </p>
      </div>
        ...
        <Card className="p-6 border-border shadow-sm bg-card space-y-4">
          ...
          <div className="pt-4 border-t border-border/50 space-y-3">
            <div className="flex items-center gap-2 text-foreground font-medium mb-2">
              ...
              {esText('conf_succ_configuration', 'Ihre Konfiguration')}
            </div>
