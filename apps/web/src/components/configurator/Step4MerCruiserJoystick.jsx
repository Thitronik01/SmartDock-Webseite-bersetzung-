...
const Step4MerCruiserJoystick = () => {
  const { merCruiserJoystick, setMerCruiserJoystick, goNext, goBack } = useConfigurator();
  const { t } = useLanguage();
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!merCruiserJoystick) return setError(t('err_select'));
    goNext();
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      <div>
        <h2 className="text-2xl font-bold mb-2">{t('s4_mer_joy')}</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
        <SelectionCard 
          title={t('step2_yes')} 
          checked={merCruiserJoystick === 'Ja'} 
          onClick={() => setMerCruiserJoystick('Ja')} 
        />
        <SelectionCard 
          title={t('s4_mer_joy_no')} 
          disabled={true} 
          checked={false} 
          onClick={() => {}} 
        />
...
export default Step4MerCruiserJoystick;