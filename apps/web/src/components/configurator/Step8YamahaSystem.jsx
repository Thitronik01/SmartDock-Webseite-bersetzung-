...
const Step8YamahaSystem = () => {
  const { yamahaSystem, setYamahaSystem, configData, goNext, goBack } = useConfigurator();
  const { t } = useLanguage();
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!yamahaSystem) return setError(t('err_select'));
    goNext();
  };

  // Strictly check the single source of truth for motorCount
  const isSingle = configData?.motorCount === 1;

  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      <div>
        <h2 className="text-2xl font-bold mb-2">{t('s8_yam_sys')}</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
        {isSingle ? (
          <SelectionCard 
            title={t('s8_yam_ind')} 
            checked={yamahaSystem === 'Helm Master EX (Single)'} 
            onClick={() => setYamahaSystem('Helm Master EX (Single)')} 
          />
        ) : (
          <SelectionCard 
            title={t('s8_yam_helm')} 
            checked={yamahaSystem === 'Helm Master EX (Dual)'} 
            onClick={() => setYamahaSystem('Helm Master EX (Dual)')} 
          />
        )}
      </div>
...
export default Step8YamahaSystem;