...
const Step11VolvoPentaSystem = () => {
  const { volvoPentaSystem, setVolvoPentaSystem, configData, goNext, goBack } = useConfigurator();
  const { t } = useLanguage();
  const [error, setError] = useState('');

  const isMultiEngine = configData.motorCount >= 2;

  const handleNext = () => {
    if (!volvoPentaSystem) return setError(t('err_select'));
    goNext();
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      <div>
        <h2 className="text-2xl font-bold mb-2">{t('s_vol_sys')}</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
        <SelectionCard
          title={t('s_vol_sys_evc_de')}
          checked={volvoPentaSystem === 'EVC -D, -E'}
          onClick={() => setVolvoPentaSystem('EVC -D, -E')}
        />
        {isMultiEngine && (
          <SelectionCard
            title={t('s_vol_sys_evc2')}
            checked={volvoPentaSystem === 'EVC 2.0'}
            onClick={() => setVolvoPentaSystem('EVC 2.0')}
          />
        )}
      </div>
...
export default Step11VolvoPentaSystem;
