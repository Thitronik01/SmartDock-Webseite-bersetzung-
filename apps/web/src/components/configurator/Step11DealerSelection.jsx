...
// Maps country strings stored in various languages to ISO 3166-1 alpha-2 codes.
// Mirrors the logic in DealerFinderPage so both finders match the same records.
...
const Step11DealerSelection = () => {
  ...
  // Auto-select NAGY Yachting for Switzerland (exclusive contract) after dealers load
  useEffect(() => {
    if (loading || dealers.length === 0) return;
    if (normalizeCountry(selectedCountry) !== 'CH') return;
    if (selectedDealer) return; // already selected (e.g., after back navigation)
    const nagy = dealers.find(d => d.name?.toLowerCase().includes('nagy'));
    if (nagy) {
      setSelectedDealer(nagy);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const parts = [
        ...
        `Dealer: ${selectedDealer.name} (${selectedDealer.country})`
      ].filter(Boolean);
      ...
      await pb.collection('configurator_orders').create({
      ...
      toast({
        title: "Fehler",
        description: t('err_send') || 'Es gab einen Fehler beim Senden deiner Anfrage.',
        variant: "destructive"
      });
    } finally {
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {COUNTRIES.sort((a, b) => a.name.localeCompare(b.name)).map(c => (
                <option key={`filter-${c.code}`} value={c.code}>{c.name}</option>
              ))}
            </select>
          </div>
          <div className="space-y-2">
...
export default Step11DealerSelection;