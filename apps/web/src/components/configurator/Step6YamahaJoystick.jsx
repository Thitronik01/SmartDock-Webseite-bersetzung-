...
const Step6YamahaJoystick = () => {
  const { yamahaJoystick, setYamahaJoystick, goNext, goBack } = useConfigurator();
  const { t } = useLanguage();
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!yamahaJoystick) return setError(t('err_select'));
    goNext();
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      <div>
        <h2 className="text-2xl font-bold mb-2">{t('s6_yam_joy')}</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
        <SelectionCard 
          title={t('step2_yes')} 
          checked={yamahaJoystick === 'Ja'} 
          onClick={() => setYamahaJoystick('Ja')} 
        />
        <SelectionCard 
          title={t('step2_no')} 
          checked={false} 
...
export default Step6YamahaJoystick;