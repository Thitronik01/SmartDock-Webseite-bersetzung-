...
const Step9VolvoPentaJoystick = () => {
  const { volvoPentaJoystick, setVolvoPentaJoystick, goNext, goBack } = useConfigurator();
  const { t } = useLanguage();
  const [error, setError] = useState('');

  const handleNext = () => {
    if (!volvoPentaJoystick) return setError(t('err_select'));
    goNext();
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      <div>
        <h2 className="text-2xl font-bold mb-2">{t('s9_vol_joy')}</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
        <SelectionCard 
          title={t('s9_vol_joy_yes')} 
          checked={volvoPentaJoystick === 'Ja, ein Joystick ist vorhanden'} 
          onClick={() => setVolvoPentaJoystick('Ja, ein Joystick ist vorhanden')} 
        />
        <SelectionCard 
          title={t('s9_vol_joy_no')} 
          checked={volvoPentaJoystick === 'Nein, nur Gashebel'} 
          onClick={() => setVolvoPentaJoystick('Nein, nur Gashebel')} 
        />
      </div>
...
export default Step9VolvoPentaJoystick;