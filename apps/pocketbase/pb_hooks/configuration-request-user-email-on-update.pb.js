...
onRecordAfterUpdateSuccess((e) => {
  ...
  const htmlBody = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
      <h2 style="color: #0f172a;">Ihre Bootkonfiguration wurde zugewiesen</h2>
      <p>Hallo ${firstName} ${lastName},</p>
      <p>wir haben Ihre Konfigurationsanfrage erfolgreich an einen unserer Partner weitergeleitet.</p>
      
      <div style="background-color: #f1f5f9; padding: 15px; border-radius: 6px; margin: 20px 0;">
        <h3 style="margin: 0; color: #0f172a;">Bearbeitungsnummer: ${referenceNumber}</h3>
      </div>

      <h3 style="color: #0f172a; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px;">Persönliche Informationen</h3>
      <p><strong>Email:</strong> ${email}</p>
      <p><strong>Telefon:</strong> ${phone}</p>
      <p><strong>Unternehmen:</strong> ${company}</p>
      <p><strong>Adresse:</strong> ${street} ${houseNumber}, ${postalCode} ${city}</p>

      <h3 style="color: #0f172a; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px; margin-top: 25px;">Bootkonfiguration</h3>
      ...
      <h3 style="color: #0f172a; border-bottom: 1px solid #e2e8f0; padding-bottom: 5px; margin-top: 25px;">Zugewiesener Händler</h3>
      <div style="background-color: #f0fdf4; padding: 15px; border-radius: 8px;">
        <p style="margin: 0;"><strong>Händler:</strong> ${dealerName}</p>
      </div>

      <p style="margin-top: 30px;">Der Händler wird sich in Kürze mit Ihnen in Verbindung setzen.</p>
  ...
  const message = new MailerMessage({
    ...
    to: [{ address: email }],
    subject: "Ihre Bootkonfiguration wurde zugewiesen - Bearbeitungsnummer: " + referenceNumber,
    html: htmlBody
  });
